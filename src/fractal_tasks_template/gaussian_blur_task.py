...
def gaussian_blur_task(
    ...
    *zarr_dir_, image_name = zarr_url.rstrip("/").split("/")
    zarr_dir = "/".join(zarr_dir_)

    output_image_name = f"{image_name}_{output_image_suffix}"
    output_image_name = output_image_name.rstrip().lstrip().replace(" ", "_")
    logging.info(f"New image name: {output_image_name}")

    new_ome_zarr_path = f"{zarr_dir}/{output_image_name}"
    new_ome_zarr = ome_zarr.derive_image(
        store=new_ome_zarr_path,
        name=output_image_name,
        overwrite=overwrite,
    )
    logging.info(f"New OME-Zarr created at {new_ome_zarr_path=}")
    input_image = ome_zarr.get_image()
    output_image = new_ome_zarr.get_image()

    if sigma_z is None:
        # If sigma_z is not provided, apply 2D Gaussian blur
        axes_order = "cyx"
        sigma = (1, sigma_xy, sigma_xy)
    else:
        # If sigma_z is provided, apply 3D Gaussian blur
        axes_order = "czyx"
        sigma = (1, sigma_z, sigma_xy, sigma_xy)
    ...
    if overwrite_input:
        ...
        os.rename(new_ome_zarr_path, zarr_url)
        shutil.rmtree(new_ome_zarr_path)
        update_dict = {"zarr_url": zarr_url}
    else:
        update_dict = {"zarr_url": new_ome_zarr_path, "origin": zarr_url}

    logging.info("Gaussian blur processing complete.")
    # Area, mean intensity, and other measurements can be computed here
